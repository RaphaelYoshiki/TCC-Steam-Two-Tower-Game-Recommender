#Este código acessa a API da plataforma de jogos Steam e cria uma base de dados contendo cada jogo a uma taxa de
...
def main():
    ...
    while len(apps_remaining_deque) > 0:
        while not check_internet():
            print_log("No internet connection. Awaiting for connection...")
            time.sleep(10)
        ...
        try:
            ...
            appusertags_req = requests.get(f"https://steamspy.com/api.php?request=appdetails&appid={appid}")

            # Se qualquer status code for 429 ou 403, trate normalmente como já faz
            if appdetails_req.status_code == 429 or appreviews_req.status_code == 429 or appusertags_req.status_code == 429:
                print_log(f'Too many requests. Put App ID {appid} back to deque. Sleep for 10 sec')
                apps_remaining_deque.appendleft(appid)
                time.sleep(10)
                continue

            if appdetails_req.status_code == 403 or appreviews_req.status_code == 403 or appusertags_req.status_code == 403:
                print_log(f'Forbidden to access. Put App ID {appid} back to deque. Sleep for 5 min.')
                apps_remaining_deque.appendleft(appid)
                time.sleep(5 * 60)
                continue

            # Se qualquer status code não for 200, reponha no deque e tente depois
            if appdetails_req.status_code != 200 or appreviews_req.status_code != 200 or appusertags_req.status_code != 200:
                print_log(f"Request failed (status code). Put App ID {appid} back to deque. Sleep for 5 sec.")
                apps_remaining_deque.appendleft(appid)
                time.sleep(5)
                continue

            # Tente decodificar o JSON, se falhar, reponha no deque e tente depois
            try:
                appdetails = appdetails_req.json()
                appdetails = appdetails[str(appid)]
                appreviews = appreviews_req.json()
                appusertags = appusertags_req.json()
            except Exception as e:
                print_log(f"JSON decode error for App ID {appid}. Put back to deque. Sleep for 30 sec.")
                error_counter += 1
                if error_counter >= 5:
                    print_log(f"Too many JSON decode errors for App ID {appid}. Added to error apps list.")
            ...
            print_log(f"Error in request for App ID {appid}. Put back to deque. Sleep for 30 sec.")
            traceback.print_exc(limit=5)
            apps_remaining_deque.appendleft(appid)
            time.sleep(30)
            continue
        ...
        if appdetails_data['type'] == "game":            
            appdetails_data['appid'] = appid
            if 'query_summary' in appreviews:
                if appreviews['query_summary'].get('total_reviews') is None:
                    excluded_apps_list.append(appid)
                    print_log(f"Number of reviews too low, added App ID: {appid} ({appdetails_data['name']}) to excluded apps list")
                elif appreviews['query_summary'].get('total_reviews') > 50:
                    appdetails_data['review_score'] = appreviews['query_summary'].get('review_score_desc', 'no_reviews')
                    appdetails_data['user_tags'] = appusertags['tags']                   
                    apps_dict[appid] = appdetails_data
                    print_log(f"Successfully get content of App ID: {appid} ({appdetails_data['name']})")
                else:
                    excluded_apps_list.append(appid)
                    print_log(f"Number of reviews too low, added App ID: {appid} ({appdetails_data['name']}) to excluded apps list")
            else:
                excluded_apps_list.append(appid)
                print_log(f"App has no review data, added App ID: {appid} ({appdetails_data['name']}) to excluded apps list")
        else:
            excluded_apps_list.append(appid)
            print_log(f"App not a game, added App ID: {appid} ({appdetails_data['name']}) to excluded apps list")

        i += 1
        # for each 500, attempt to save checkpoints
        if i >= 500:
